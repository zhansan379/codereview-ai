...
export default {
  'zh-CN': {
    ...
    staticTitle: '静态分析',
    staticIntro: '审查时额外跑 ruff / semgrep 等静态检查工具，把工具发现的问题单独列出来；保存后立即生效。',
    staticTip: {
      what: '静态分析 = 用规则引擎机械地扫代码（语法错误、常见漏洞模式），不调用 AI。',
      when: '默认关闭：AI 审查已能覆盖多数问题；需要工具级精确扫描（如安全规则）时再开。',
    },
    staticLabel: '启用静态分析',
    staticHint: '开 = 每次审查附带跑一次静态分析（更慢）',
    staticOn: '已开启静态分析（即时生效）',
    staticOff: '已关闭静态分析（即时生效）',
    saveStatic: '保存静态分析设置',
    staticSourceLabel: '开关来源',
  ...
  en: {
    ...
    staticTitle: 'Static analysis',
    staticIntro: 'Run ruff / semgrep etc. alongside each review and list tool findings separately; saving takes effect immediately.',
    staticTip: {
      what: 'Static analysis scans code with rule engines (syntax errors, common vulnerability patterns) — no AI involved.',
      when: 'Off by default: AI review already covers most issues; enable when you need tool-grade precision (e.g. security rules).',
    },
    staticLabel: 'Enable static analysis',
    staticHint: 'On = every review also runs static analysis (slower)',
    staticOn: 'Static analysis enabled (effective immediately)',
    staticOff: 'Static analysis disabled (effective immediately)',
    saveStatic: 'Save static analysis setting',
    staticSourceLabel: 'Toggle source',

...
class ReviewRepository:
    ...
    async def set_exec_metrics(
        ...
        *,
        ...
        """写执行态快照列（仪表盘 agent/diff 区分 + 复杂度×成本散点依据）。

        `exec_mode` 记录**实际跑通**的路径（agentic 降级时由调用方落 'diff'；
        未真正执行审查时不传/传 None，落库为 NULL，统计与展示时排除）。
        `diff_additions/deletions`：增删行分开记（成员分析「+N / -M」）；不传保持 NULL
        （旧调用点/存量行），成员分析按「列 NULL → diff_snapshot 解析 → 合计兜底」退化。
        """
        session = session_factory(self._engine)
        async with session() as s:
            row = (await s.execute(
            ...
            if diff_additions is not None:
                row.diff_additions = diff_additions
            if diff_deletions is not None:
                row.diff_deletions = diff_deletions
            await s.commit()

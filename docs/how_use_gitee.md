...
从零把 Gitee 仓库接进审查：申请私人令牌 → 配进本服务 → 新增项目 → 配置 webhook（或主动补拉）→ 验证。GitHub 接入见 [GitHub 接入教程](how_use_github.md)。
...
3. **WebHook 密码/签名验证**：勾选并填入服务端 `CR_WEBHOOK_SECRET` 的值（就是 `.env` 中配置的那个）。Gitee 会用该密码对每次请求做 HMAC-SHA256 签名（`X-Gitee-Token` + `X-Gitee-Timestamp` 请求头），服务端校验不过返回 401，所以**必须一致**；
